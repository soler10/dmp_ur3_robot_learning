...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<LearnDMPFromDemo-response>)))
  ...
  (cl:format cl:nil "~%# Returns a DMP for each DOF, intended to be linked together with a single phase variable~%DMPData[] dmp_list~%~%# A time constant (in seconds) that will cause the DMPs to replay at the same speed they were demonstrated. ~%float64 tau~%~%~%================================================================================~%MSG: dmp/DMPData~%float64 k_gain~%float64 d_gain~%float64[] weights~%float64[] f_domain~%float64[] f_targets~%~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'LearnDMPFromDemo-response)))
  "Returns full string definition for message of type 'LearnDMPFromDemo-response"
  (cl:format cl:nil "~%# Returns a DMP for each DOF, intended to be linked together with a single phase variable~%DMPData[] dmp_list~%~%# A time constant (in seconds) that will cause the DMPs to replay at the same speed they were demonstrated. ~%float64 tau~%~%~%================================================================================~%MSG: dmp/DMPData~%float64 k_gain~%float64 d_gain~%float64[] weights~%float64[] f_domain~%float64[] f_targets~%~%~%~%"))
